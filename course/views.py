...
def course_list(request,title=None):
    ...
    return render(request, 'course/short-course-view.html', {'courses': courses, 'query': title})
...
def create_course(request):
    if request.method == 'POST':
        ...
        print(form,"111111111111111111",form.is_valid,form.errors)
        if form.is_valid():
            # Create a new Course object with the form data
            form.save()
            return redirect('course_list')
        else:
            print(form.errors,"Error occured")
    else:
        form = CourseForm()
...
def search_courses(request):
    ...
    print("JsonResponse========",JsonResponse(list(results), safe=False))
